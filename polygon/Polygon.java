...
public class Polygon extends Chain {
	...
	public Polygon transform(Transformation T){
		Polygon transformed = Polygon.makeEmpty();
		for(int i=0; i < this.length(); i++)
			transformed.add(T.transformVertex3D(this.get(i)));
		return transformed;
	}
	
	public Polygon transform_only_point(Transformation T){
		Polygon transformed = Polygon.makeEmpty();
		for(int i=0; i < this.length(); i++)
			transformed.add(T.transformVertex3D_only_point(this.get(i)));
		return transformed;
	}
	
	public Polygon euclideanXY(){
		Polygon cartesian = Polygon.makeEmpty();
		for(int i=0; i < this.length(); i++)
			cartesian.add(this.get(i).euclideanXY());
		return cartesian;
	}
